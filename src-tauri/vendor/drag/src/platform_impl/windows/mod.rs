    ...
    if let Ok(RawWindowHandle::Win32(_w)) = handle.window_handle().map(|h| h.as_raw()) {
        match item {
            DragItem::Files(files) => {
                ...
                for f in files {
                    // dunce::canonicalize stalls or fails on slow/transient
                    // network mounts. Fall back to the original path so the
                    // user can still drag NAS files.
...
fn get_file_data_object(paths: &[PathBuf]) -> Option<IDataObject> {
    unsafe {
        // Network/UNC paths can make ILCreateFromPathW return NULL, which
        // makes SHCreateShellItemArrayFromIDLists crash on null deref.
        // Return None gracefully so the caller can fall back.
...
fn get_shell_item_array(paths: &[PathBuf]) -> Option<IShellItemArray> {
    unsafe {
        let list: Vec<*const Common::ITEMIDLIST> = paths
            ...
            .collect();
        // Bail if the shell couldn't resolve every path — passing a NULL
        // ITEMIDLIST to SHCreateShellItemArrayFromIDLists is a hard crash.
        if list.iter().any(|p| p.is_null()) {
            return None;
        }
